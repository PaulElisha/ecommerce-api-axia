...
export class MulticallSwap {
  ...
  private readonly UNISWAP_FACTORY_ADDRESS: string =
    "0x7431A23897ecA6913D5c81666345D39F27d946A4"; // Update to DragonSwap factory address if different
  ...
  private async approveToken(amountIn: bigint, tokenIn: string): Promise<void> {
    ...
    console.log(
      `Approval transaction sent: ${approvalTx.hash} for ${amountIn}`
    );
  ...
  private async getPoolFee(tokenIn: string, tokenOut: string): Promise<number> {
    ...
    for (const fee of feeTiers) {
      ...
      if (poolAddress !== ethers.ZeroAddress) {
        console.log(
          `Valid pool found for ${tokenIn}-${tokenOut} with fee: ${fee}`
        );
        return fee;
      }
    }
    ...
    throw new Error(
      `No valid pool found for tokens ${tokenIn} and ${tokenOut}`
    );
  }
   ...
   */
  private async poolExistsWithLiquidity(
    tokenIn: string,
    tokenOut: string,
    fee: number
  ): Promise<boolean> {
  ...
  public async performSwaps(params: SwapParam[]): Promise<void> {
    ...
    for (const param of params) {
      ...
      if (
        !(await this.poolExistsWithLiquidity(
          param.tokenIn,
          param.tokenOut,
          param.fee
        ))
        ...
        throw new Error(
          `No liquidity available in the pool for tokens ${param.tokenIn} and ${param.tokenOut}`
        );
      }

      const slippageAdjustedAmountOutMin = this.calculateSlippage(
        param.amountOutMinimum,
        param.slippageTolerance
      );
      param.amountOutMinimum = Number(slippageAdjustedAmountOutMin);

      const encData = this.swapRouterContract.interface.encodeFunctionData(
        "exactInputSingle",
        [param]
      );
    ...
    const swapCalldata = this.swapRouterContract.interface.encodeFunctionData(
      "multicall",
      [callData]
    );
